...
def process_peak_data(adata, n_top_features=20000):
    """
    处理已加载的Peak数据：TF-IDF归一化（等效Signac RunTFIDF）+ 高变特征筛选（等效Signac FindTopFeatures）
    参数:
        adata: 包含peak数据的AnnData对象
        n_top_features: 要筛选的高变peak数量，默认20000
    返回:
        处理后的AnnData对象
    """
    print("=" * 50)
...
def process_peak_data_path(peak_h5ad_path, n_top_features=20000):
    """
    通过文件路径读取Peak数据→TF-IDF归一化→高变Peak筛选（与process_peak_data功能一致，支持路径输入）
    参数:
        peak_h5ad_path: peak数据的h5ad文件路径（字符串）
        n_top_features: 要筛选的高变peak数量，默认20000
    返回:
        处理后的AnnData对象
    """
