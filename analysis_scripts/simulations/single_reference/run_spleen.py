...
def set_base_config():
    base_config = {
        "sc_rna_path": "path/to/input.h5ad",
        "st_rna_path": "path/to/input.h5ad",
        "st_adt_path": "path/to/input.h5ad",
        "output_dir": "outputs/single_reference_spleen/SMODER",
        "model_save_dir": os.path.join("outputs/single_reference_spleen/SMODER",
                                       "trained_models"),
    }
    # 自动创建输出目录
...
def set_analysis_params():
    params = {
        # ---------------------- 1. 数据预处理参数 ----------------------
        "ref_celltype_col": "cell_type",  # 单细胞参考数据的细胞类型列名
        "sample_id_col": "batch",           # 单细胞参考数据的样本列名（用于计算Basis矩阵）
        "log_FC": 1.25,                      # 筛选信息基因的logFC阈值
        "do_select_info_genes": True,        # 是否筛选信息基因
        "ct_select": None,                   # 需保留的细胞类型（None表示保留所有）
        "modal2_type": "adt",               # 第二模态类型（adt/peak，对应模型内置预处理）
        ...
        "modal2_target_dim": 128,            # 第二模态最终目标维度
        ...
        "weight_consistency": 0.01,             # 一致性损失权重
        "weight_spatial": 0,              # 空间平滑损失权重
        "seed": 1,                           # 随机种子（确保可复现）
        "model_save": True,                  # 是否保存训练后的模型
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Run SMODER on a single-reference simulated dataset."
    )
    parser.add_argument(
        "--sc-rna",
        default="path/to/spleen/sc.h5ad",
        help="Path to the single-cell RNA reference .h5ad file.",
    )
    parser.add_argument(
        "--st-rna",
        default="outputs/single_reference_spleen/simulation_rna.h5ad",
        help="Path to the simulated spatial RNA .h5ad file.",
    )
    parser.add_argument(
        "--st-modality2",
        default="outputs/single_reference_spleen/simulation_adt.h5ad",
        help="Path to the simulated second-modality .h5ad file.",
    )
    parser.add_argument(
        "--output-dir",
        default="outputs/single_reference_spleen/SMODER",
...
if __name__ == "__main__":
    ...
    base_config["sc_rna_path"] = args.sc_rna
    base_config["st_rna_path"] = args.st_rna
    base_config["st_adt_path"] = args.st_modality2
    base_config["output_dir"] = args.output_dir
    base_config["model_save_dir"] = os.path.join(args.output_dir, "trained_models")
    os.makedirs(base_config["output_dir"], exist_ok=True)
    os.makedirs(base_config["model_save_dir"], exist_ok=True)
