...
# ====================== 全局配置 ======================
# 路径配置
RAW_DATA_PATH = "path/to/mousebrain/reference_data"
OUT_PATH = "outputs/single_reference_mousebrain"
# Output directory is created after parsing command-line arguments.

# RNA参考数据文件
RNA_REF_FILE = "sc_mousebrain_processed.h5ad"
CELL_TYPE_COL = "cell_type"

# ATAC Peak生成参数
TOTAL_PEAKS = 20000  # 全局peak总数
CORE_PEAKS_PER_CT = 100  # 每个细胞类型核心峰数量
WEAK_PEAKS_PER_CT = 900  # 每个细胞类型弱信号峰数量
CORE_WEIGHT = 1.0  # 核心峰权重
WEAK_WEIGHT_MIN = 0.05  # 弱峰最小权重
WEAK_WEIGHT_MAX = 0.25  # 弱峰最大权重
RANDOM_SEED = 42  # 固定随机种子（和参考代码第一轮种子一致）
...
CELL_NUMBER_MEAN = [1] * 8
CELL_TYPE_NUMBER = [3, 2, 3, 2, 4, 2, 4, 4]
...
def generate_20k_peaks_atac(
        ...
        cell_type_col,
        total_peaks=20000,
        core_peaks_per_ct=100,
        weak_peaks_per_ct=900,
        core_weight=1,
        weak_weight_min=0.05,
        weak_weight_max=0.25,
        seed=42,
        noise_scale=0.3
):
    """
    ...
    print(f"   - ATAC值范围：[{adata_atac.X.min():.4f}, {adata_atac.X.max():.4f}]")
