...
def set_base_config():
    base_config = {
        ...
        "output_dir": "outputs/single_reference_hgm/SMODER",
        "model_save_dir": os.path.join("outputs/single_reference_hgm/SMODER",
                                       "trained_models"),
    }
    # 自动创建输出目录
...
def set_analysis_params():
    params = {
        # ---------------------- 1. 数据预处理参数 ----------------------
        "ref_celltype_col": "Assignment",  # 单细胞参考数据的细胞类型列名
        "sample_id_col": "Patient",           # 单细胞参考数据的样本列名（用于计算Basis矩阵）
...
def parse_args():
    ...
    parser.add_argument(
        "--sc-rna",
        default="path/to/HGM/sc.h5ad",
        help="Path to the single-cell RNA reference .h5ad file.",
    )
    parser.add_argument(
        "--st-rna",
        default="outputs/single_reference_hgm/simulation_rna.h5ad",
        help="Path to the simulated spatial RNA .h5ad file.",
    )
    parser.add_argument(
        "--st-modality2",
        default="outputs/single_reference_hgm/simulation_adt.h5ad",
        help="Path to the simulated second-modality .h5ad file.",
    )
    parser.add_argument(
        "--output-dir",
        default="outputs/single_reference_hgm/SMODER",
